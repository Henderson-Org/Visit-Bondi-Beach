...
export function WhatsOnLandingView({
  ...
  return (
    <div>
      ...
      {events.length > 0 && (
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(itemListJsonLd(h1, events.map((r) => ({ name: r.event.title, description: r.event.summary })))) }} />
      )}
