...
export interface RouteStop {
  label: string;
  sub?: string;
  /** Optional internal link to the stop's own page (e.g. a location page). */
  href?: string;
}
  ...
  return (
    <section aria-label={title || 'Route'} className="rounded-2xl border border-sand-200 bg-white p-5 sm:p-6">
      ...
      <ol className={`${title ? 'mt-4' : ''} flex overflow-x-auto pb-2`}>
        {stops.map((s, i) => {
          const label = s.href ? (
            <Link href={s.href} className="text-ocean-700 hover:underline">{s.label}</Link>
          ) : (
            s.label
          );
          return (
            <li key={s.label} className="relative flex min-w-[7rem] flex-1 flex-col items-center px-2 text-center sm:min-w-0">
              ...
              <p className="mt-2 text-sm font-medium leading-tight text-ink-900">{label}</p>
