...
> **⚠️ Read `OWNER-INPUT-REQUIRED.md` first.** There is one structural decision that blocks the *build* phase (this migration is currently being staged inside the unrelated `japan-travel` repository). The audit below is valid regardless of that decision.
...
- **Wrong repository (blocking the build).** See `OWNER-INPUT-REQUIRED.md` #1. The migration should live in its own repo, not `japan-travel`.
- **No access yet** to Squarespace admin, DNS, Google Analytics, or Search Console — so backlink-informed decisions (which tags to keep, which junk URLs matter) and the final DNS/email preservation steps cannot be completed. None of these block the audit or the staging build.
- **Image licensing** — Squarespace-hosted images must be re-hosted; confirm the owner holds rights before re-serving them.
